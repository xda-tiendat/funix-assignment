...
public class AddPostController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            boolean result = postDAO.doInsertPost(title, summary, content, username, timepost, optradio);
            if (result != false) {
                request.setAttribute(Constants.MES_AUTHOR, "Inserted");
            } else {
                request.setAttribute(Constants.MES_AUTHOR, "Insert Fail");
            }
            
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            RequestDispatcher rd = request.getRequestDispatcher("Authoring.jsp");
